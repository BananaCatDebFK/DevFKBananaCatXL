loadstring(game:HttpGet("https://raw.githubusercontent.com/BananaCatDebFK/DevFKBananaCatXL/refs/heads/main/Hook.lua"))()
...
if game.Players.LocalPlayer.Character[Weapon].Parent.Parent then game.Players.LocalPlayer.Character[Weapon].Parent = game.Players.LocalPlayer.Backpack end
...
if Noclip.Parent.Parent then Noclip.Parent = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart end
    ...
    for _, otherPlayer in ipairs(players) do
        if otherPlayer ~= player and otherPlayer.Character then
            ...
            if rootPart and IsEntityAlive(otherPlayer.Character) then
                local distance = (rootPart.Position - playerPos).Magnitude
                if distance <= range then table.insert(targets, otherPlayer.Character) end
            end
        end
    end
    return targets
...
Toggle:OnChanged(function(Value)
    ...
    if Value then
        if not getgenv().rejoin then
            getgenv().rejoin = game:GetService("CoreGui"):WaitForChild("RobloxPromptGui", 5)            
            if getgenv().rejoin then
                getgenv().rejoin = getgenv().rejoin.promptOverlay.ChildAdded:Connect(function(child)
...
LGa:AddButton({
    ...
    Callback = function()
        ...
        for i, v in pairs(fruitObjects) do
            ...
            count = count + 1
if v.Parent.Parent then v.Parent = game.Workspace.Map end
...
if v.Parent.Parent then v.Parent = game.Players.LocalPlayer.Backpack end
...
if target.Parent.Humanoid.WalkSpeed.Parent then target.Parent.Humanoid.WalkSpeed = 0 end
...
if BillboardGui.Parent.Parent then BillboardGui.Parent = Parent end
    BillboardGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    BillboardGui.AlwaysOnTop = true
    BillboardGui.Size = UDim2.new(0, 200, 0, 50)
    BillboardGui.StudsOffset = Vector3.new(0, 2.5, 0)
if TextLabel.Parent.Parent then TextLabel.Parent = BillboardGui end
    ...
    while task.wait(0.1) do
        if getgenv().AutoDefendVolcano then
            ...
            if currentTarget then
                local targetPosition = currentTarget.Position + Vector3.new(0, 30, 0)                
...
task.spawn(function()
    local prevState = false  
    while true do
        if getgenv().LockMoonAndOnRaceV3 and World3 then
            pcall(function()
                local moonDir = game.Lighting:GetMoonDirection()
                local lookAtPos = game.Workspace.CurrentCamera.CFrame.Position + moonDir * 100
                local currentLookAt = game.Workspace.CurrentCamera.CFrame.LookVector
                if (lookAtPos - game.Workspace.CurrentCamera.CFrame.Position).unit ~= currentLookAt then
                    game.Workspace.CurrentCamera.CFrame = CFrame.lookAt(game.Workspace.CurrentCamera.CFrame.Position, lookAtPos)
                end
                if getgenv().LockMoonAndOnRaceV3 ~= prevState then
                    game:GetService("ReplicatedStorage").Remotes.CommE:FireServer("ActivateAbility")
                    prevState = getgenv().LockMoonAndOnRaceV3
                end
            end)
        end
        task.wait(0.5)
        ...
        while task.wait(0.1) do
            if getgenv().AutoTrialRace and World3 then
                ...
                elseif race == "Skypiea" then
                    ...
                    if humanoidRootPart and (c1.Position - humanoidRootPart.Position).Magnitude <= 2000 then
                        topos(c1.CFrame)
                    end
                    ...
                    if seaBeast then
                        ...
                        if rootPart then
                            ...
                            for _, toolType in ipairs(toolTypes) do
                                ...
                                local keyCodes = { 122, 120, 99 }
                                for _, keyCode in ipairs(keyCodes) do
                                    game:GetService("VirtualInputManager"):SendKeyEvent(true, keyCode, false, humanoidRootPart)
                                    game:GetService("VirtualInputManager"):SendKeyEvent(false, keyCode, false, humanoidRootPart)
                                    wait(0.1)
                                end
                            end
                        end
                ...
                elseif race == "Mink" then
                    for _, v in pairs(game:GetService("Workspace"):GetChildren()) do
                        if v.Name == "StartPoint" and humanoidRootPart then
